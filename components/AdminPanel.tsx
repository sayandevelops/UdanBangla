...
export const AdminPanel: React.FC<AdminPanelProps> = ({ topics, onExit }) => {
  ...
  const handleManualSubmit = (e: React.FormEvent) => {
    ...
    addQuestionToBank(selectedTopicId, newQuestion);
  ...
  const handleCsvUpload = () => {
    ...
    try {
      ...
      bulkAddQuestionsToBank(selectedTopicId, questions);
      showNotification('success', `Uploaded ${questions.length} questions successfully`);
      setCsvContent('');
    } catch (err) {
      showNotification('error', 'Failed to parse CSV');
    }
  };

  const currentTopicQuestions = getQuestionsForTopic(selectedTopicId);
                ...
                <button 
                  onClick={() => {
                    if (confirm('Are you sure you want to delete all questions for this topic?')) {
                      clearTopicQuestions(selectedTopicId);
